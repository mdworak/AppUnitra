...
public class DatabaseInne extends SQLiteOpenHelper {
    public DatabaseInne(Context context) {
        super(context, "inne.db", null, 1);
    }
    ...
    public void addInne(DataInne data){
        ...
        db.insertOrThrow("inne",null, values);
    }

    public List<DataInne> getAll(){
        List<DataInne> contacts = new LinkedList<DataInne>();
        String[] columns={"nazwa","producent","rodzaj","lampy","rok_produkcji","wymiary","waga","dodinfo"};
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor =db.query("inne",columns,null,null,null,null,null,null);
        while(cursor.moveToNext()){
            DataInne data = new DataInne();
    ...
    public String getModel(String name){
        ...
        String args[]={name+""};
        Cursor cursor=db.query("inne",columns,"nazwa=?",args,null,null,null,null);
        String model=null;
        if(cursor!=null){
            cursor.moveToFirst();
    ...
    public String getNazwa(String id){
        ...
        String[] columns={"nazwa"};
        String args[]={id+""};
        Cursor cursor=db.query("inne",columns,"id=?",args,null,null,null,null);
        String model=null;
        if(cursor!=null){
            cursor.moveToFirst();
            model = cursor.getString(0);
    ...
    public List<DataInne> getDane(String id){
        List<DataInne> contacts = new LinkedList<DataInne>();
        String[] columns={"nazwa","producent","rodzaj","lampy","rok_produkcji","wymiary","waga","dodinfo"};
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery(
                "SELECT nazwa,producent,rodzaj,lampy,rok_produkcji,wymiary,waga,dodinfo  FROM  inne WHERE id='"+id+"'" , null);
    ...
    public int getID(String nazwa) {
        ...
        Cursor mCursor = db.rawQuery(
                "SELECT id  FROM  inne WHERE nazwa='"+nazwa+"'" , null);
    ...
    public String[] getZdjecia(String id){
        ...
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery(
                "SELECT zdjecie1,zdjecie2,zdjecie3  FROM  inne WHERE id='"+id+"'" , null);
        cursor.moveToFirst();
